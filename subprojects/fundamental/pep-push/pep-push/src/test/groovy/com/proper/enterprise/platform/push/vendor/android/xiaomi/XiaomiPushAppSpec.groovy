...
class XiaomiPushAppSpec extends Specification {
    ...
    def "testXiaomiPushAppParamTooLong"() {
        def appKey = 'MobileOADev'
        // Token of test device
        def token = 'UWaUVPwMtRaZ/X+vSBt0MUkBD/AfrS9PnhhOikuLkvALDOpZG7+549QF7+u9sDmP'
        def msgTitle = 'aaaaaaaaaaaaaaaa'
        def msgContent = "$msgTitle ${System.getProperty('os.arch')} push this notification to test Xiaomi push app at ${new Date().format('yyyy-MM-dd HH:mm:ss')} in test case abcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcd"
