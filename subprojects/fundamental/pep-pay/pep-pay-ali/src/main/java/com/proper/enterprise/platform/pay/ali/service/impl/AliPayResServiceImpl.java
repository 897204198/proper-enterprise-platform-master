...
public class AliPayResServiceImpl implements AliPayResService {
    ...
    public String checkUrl(String urlvalue) throws IOException {
        ResponseEntity<byte[]> entity = HttpClient.get(urlvalue);
        if (null == entity) {
            throw new ErrMsgException("checkUrl error entity is null");
        }
        byte[] body = entity.getBody();
        if (null == body) {
            throw new ErrMsgException("checkUrl error response body is null");
        }
        return new String(body, PEPPropertiesLoader.load(CoreProperties.class).getCharset());
    }
}
