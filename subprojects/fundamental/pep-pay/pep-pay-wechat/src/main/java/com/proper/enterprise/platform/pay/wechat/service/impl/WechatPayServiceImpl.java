...
public class WechatPayServiceImpl extends AbstractPayImpl implements PayService, WechatPayService {
    ...
    protected <T extends PayResultRes, R extends OrderReq> T savePrepayImpl(R req)  throws Exception {
        ...
        try {
            ...
            uoReq.setSpbillCreateIp(payWechatProperties.getSpbillCreateIp());
            // 微信支付异步通知地址
            uoReq.setNotifyUrl(payWechatProperties.getUrlNotify());
            // 请求微信API接口
            WechatOrderRes res = getWechatRes(uoReq, payWechatProperties.getUrlUnified(),
            ...
            if (STR_SUCCESS.equals(res.getReturnCode()) && STR_SUCCESS.equals(res.getResultCode())) {
                ...
                // 随机字符串
                payObj.setNoncestr(RandomStringUtils.randomAlphabetic(payWechatProperties.getRandomLen()));
                // 时间戳
    ...
    protected <T> T getPayQueryRes(String outTradeNo) {
        ...
        req.setNonceStr(RandomStringUtils.randomAlphabetic(payWechatProperties.getRandomLen()));
        SignAdapter signAdapter = new SignAdapter();
        WechatPayQueryRes queryRes = null;
        try {
            String sign = signAdapter.marshalObject(req, WechatPayQueryReq.class);
            req.setSign(sign);
            queryRes = getWechatRes(req, payWechatProperties.getUrlOrderQuery(), "unmarshallWechatPayQueryRes", false);
        } catch (Exception e) {
    ...
    protected <T> T saveRefundProcess(T refundBody) throws Exception {
        ...
        try {
            ...
            res = getWechatRes(wechatRefundReq, payWechatProperties.getUrlRrefundQuery(), "unmarshallWechatRefundRes", true);
            // 输出LOG
    ...
    protected <T> T getRefundQueryRes(String orderNo, String refundNo) {
        ...
        try {
            String sign = signAdapter.marshalObject(req, WechatPayQueryReq.class);
            req.setSign(sign);
            queryRes = getWechatRes(req, payWechatProperties.getUrlRrefundQuery(), "unmarshallWechatRefundQueryRes", false);
        } catch (Exception e) {
    ...
    protected <T> T getBillProcess(BillReq billReq) throws Exception {
        ...
        try {
            ...
            // 随机字符串
            wechatBillReq.setNonceStr(RandomStringUtils.randomAlphabetic(payWechatProperties.getRandomLen()));
            // 签名
            SignAdapter signAdapter = new SignAdapter();
