...
class WorkflowExceptionTest extends WorkflowAbstractTest {
    ...
    private static final String WORKFLOW_NO_OUTGOING_EXCEPTION_KEY = "workflowNoOutGoingException"
    ...
    @Sql(["/com/proper/enterprise/platform/workflow/identity.sql",
        "/com/proper/enterprise/platform/workflow/datadics.sql",
        "/com/proper/enterprise/platform/workflow/wfIdmQueryConf.sql"])
    void workflowNoOutGoingExceptionTest() {
        setCurrentUserId("admin")
        def map = [:]
        map['vacationType'] = 'D'
        assert I18NUtil.getMessage("workflow.task.no.outgoing.sequence.flow") ==
            post('/workflow/process/' + WORKFLOW_NO_OUTGOING_EXCEPTION_KEY, JSONUtil.toJSON(map),
                HttpStatus.INTERNAL_SERVER_ERROR).getResponse().getContentAsString()

        map['vacationType'] = 'B'
        start(WORKFLOW_NO_OUTGOING_EXCEPTION_KEY, map)
        Map task = getTask("step1")
        assert I18NUtil.getMessage("workflow.task.no.outgoing.sequence.flow") == post('/workflow/task/' + task.taskId, JSONUtil.toJSON(map),
            HttpStatus.INTERNAL_SERVER_ERROR).getResponse().getContentAsString()
    }
}
