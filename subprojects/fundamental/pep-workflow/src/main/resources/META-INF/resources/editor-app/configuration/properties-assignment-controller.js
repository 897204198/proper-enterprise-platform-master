...
angular.module('flowableModeler').controller('FlowableAssignmentPopupCtrl',
    ...
    $scope.assignmentOptions = [
        ...
        {id: "roles", title: $translate.instant('PROPERTY.ASSIGNMENT.IDM.DROPDOWN.ROLES')}
    ];
    ...
    if (!$scope.assignmentOption) {
        ...
        if ($scope.assignment.idm) { //fill the IDM tab
            ...
            if ($scope.assignment.idm.candidateRoles && $scope.assignment.idm.candidateRoles.length > 0) {
                for (var i = 0; i < $scope.assignment.idm.candidateRoles.length; i++) {
                    $scope.popup.assignmentObject.idm.candidateRoles.push($scope.assignment.idm.candidateRoles[i]);
                }
            }
        }
    }
    ...
    if ($scope.assignment.candidateRoles && $scope.assignment.candidateRoles.length > 0) {
        for (var i = 0; i < $scope.assignment.candidateRoles.length; i++) {
            $scope.popup.assignmentObject.static.candidateRoles.push($scope.assignment.candidateRoles[i]);
        }
    }
    ...
    $scope.updateRoleFilter = function() {
        if ($scope.popup.oldRoleFilter == undefined || $scope.popup.oldRoleFilter != $scope.popup.roleFilter) {
            if (!$scope.popup.roleFilter) {
                $scope.popup.oldRoleFilter = '';
            } else {
    ...
    $scope.confirmRole = function(role) {
        if (!role) {
            // Selection is done with keyboard, use selection index
            var roles = $scope.popup.roleResults;
            if ($scope.popup.selectedRoleIndex >= 0 && $scope.popup.selectedRoleIndex < roles.length) {
                role = roles[$scope.popup.selectedRoleIndex];
            }
        }

        if (role) {
            // Only add if not yet part of candidate roles
            var found = false;
            if ($scope.popup.assignmentObject.idm.candidateRoles) {
                for (var i = 0; i < $scope.popup.assignmentObject.idm.candidateRoles.length; i++) {
                    if ($scope.popup.assignmentObject.idm.candidateRoles[i].id === role.id) {
                        found = true;
                        break;
                    }
                }
    ...
    $scope.addCandidateRole = function(role) {
        $scope.popup.assignmentObject.idm.candidateRoles.push(role);
    };

    $scope.removeCandidateRole = function(role) {
        var roles = $scope.popup.assignmentObject.idm.candidateRoles;
        var indexToRemove = -1;
    ...
    $scope.resetRoleSelection = function() {
        if ($scope.popup.roleResults && $scope.popup.roleResults.length > 0) {
            $scope.popup.selectedRoleIndex = 0;
        } else {
            $scope.popup.selectedRoleIndex = -1;
        }
    };

    $scope.nextRole = function() {
        var roles = $scope.popup.roleResults;
        if (roles && roles.length > 0 && $scope.popup.selectedRoleIndex < roles.length -1) {
            $scope.popup.selectedRoleIndex += 1;
        }
    };

    $scope.previousRole = function() {
        var roles = $scope.popup.roleResults;
        if (roles && roles.length > 0 && $scope.popup.selectedRoleIndex > 0) {
            $scope.popup.selectedRoleIndex -= 1;
        }
    };
    ...
    $scope.addCandidateRoleValue = function(index) {
        $scope.popup.assignmentObject.static.candidateRoles.splice(index + 1, 0, {value: ''});
    };

    $scope.removeCandidateRoleValue = function(index) {
        $scope.popup.assignmentObject.static.candidateRoles.splice(index, 1);
    };
    ...
    $scope.save = function () {
        ...
        if ('idm' === $scope.popup.assignmentObject.type) { // IDM
            ...
            } else if ('roles' == idm.type) {
                if ($scope.popup.assignmentObject.idm.candidateRoles && $scope.popup.assignmentObject.idm.candidateRoles.length > 0) {
                    idm.candidateRoles = $scope.popup.assignmentObject.idm.candidateRoles;
                }
            }
        ...
        if ('static' === $scope.popup.assignmentObject.type) { // IDM
            ...
            $scope.assignment.candidateRoles = $scope.popup.assignmentObject.static.candidateRoles;
        }
    ...
    var handleAssignmentInput = function ($assignment) {
        ...
        if ($assignment.candidateRoles) {
            toRemoveIndexes = [];
            for (i = 0; i < $assignment.candidateRoles.length; i++) {
              if (isEmptyString($assignment.candidateRoles[i].value)) {
                toRemoveIndexes[toRemoveIndexes.length] = i;
              }
            }
        
            if (toRemoveIndexes.length == $assignment.candidateRoles.length) {
              $assignment.candidateRoles = undefined;
            } else {
              removedItems=0;
              for (i = 0; i < toRemoveIndexes.length; i++) {
                $assignment.candidateRoles.splice(toRemoveIndexes[i]-removedItems, 1);
                removedItems++;
              }
            }
        }
    ...
    function initStaticContextForEditing($scope) {
        ...
        if (!$scope.popup.assignmentObject.static.candidateRoles || $scope.popup.assignmentObject.static.candidateRoles.length==0) {
          $scope.popup.assignmentObject.static.candidateRoles = [{value: ''}];
        }
    }
}]);
