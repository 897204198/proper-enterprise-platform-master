...
class FileControllerTest extends AbstractJPATest {
    ...
    void uploadDirTest() {
        ...
        assert "The folder is exist" == post("/file/dir", JSONUtil.toJSON(data3), HttpStatus.INTERNAL_SERVER_ERROR).getResponse().getContentAsString()

        data3['fileName'] = '测试一下最大文件名'
        assert "The file or folder name is too long" == post("/file/dir", JSONUtil.toJSON(data3), HttpStatus.INTERNAL_SERVER_ERROR).getResponse().getContentAsString()
    ...
    void putDirTest() {
        ...
        assert "The folder is exist" == put("/file/dir/" + result, JSONUtil.toJSON(fileVO), HttpStatus.INTERNAL_SERVER_ERROR).getResponse().getContentAsString()
