...
angular.module('activitiModeler').
    directive('assignmentSelect', ['$http', '$templateCache', function ($http, $templateCache) {
        ...
        directive.controller = ['$rootScope', '$scope', '$translate', '$element', function ($rootScope, $scope, $translate, $element) {
            ...
            $scope.$watch('assignmentOption', function (newValue) {
                if (newValue) {
                    ...
                    if (newValue.id == 'initiator') {
                        ...
                        $scope.assignmentObject.candidateRoles = [];
                        $scope.assignmentObject.candidateRoleFields = [];
                    ...
                    } else if (newValue.id == 'roles') {
                        // Create new empty array of roles
                        if (!$scope.assignmentObject.candidateRoles) {
                            $scope.assignmentObject.candidateRoles = [];
                        }
                        if (!$scope.assignmentObject.candidateRoleFields) {
                            $scope.assignmentObject.candidateRoleFields = [];
                        }
                    }
                } else {
            ...
            $scope.addCandidateRole = function (role, roleField) {

                // Only add it when it's not yet part of the current users
                var exists = false;
                if (role) {
                    if ($scope.assignmentObject.candidateRoles) {
                        for (var i = 0; i < $scope.assignmentObject.candidateRoles.length; i++) {
                            if ($scope.assignmentObject.candidateRoles[i].id === role.id) {
                                exists = true;
                                break;
                            }
                        }
                ...
                if (roleField) {
                    if ($scope.assignmentObject.candidateRoleFields) {
                        for (var i = 0; i < $scope.assignmentObject.candidateRoleFields.length; i++) {
                            if ($scope.assignmentObject.candidateRoleFields[i].id === roleField.id) {
                                exists = true;
                                break;
                            }
                        }
                ...
                if (role && !exists) {
                    $scope.assignmentObject.candidateRoles.push(role);
                } else if (roleField && !exists) {
                    $scope.assignmentObject.candidateRoleFields.push(roleField);
                }

            };

            $scope.removeCandidateRole = function (role) {
                var roles = $scope.assignmentObject.candidateRoles;
                var indexToRemove = -1;
            ...
            $scope.removeCandidateRoleField = function (roleField) {
                var roles = $scope.assignmentObject.candidateRoleFields;
                var indexToRemove = -1;
                for (var i = 0; i < roles.length; i++) {
                    if (roleField.id == roles[i].id) {
                        indexToRemove = i;
                        break;
                    }
                }
